...
class OrderService
{
    ...
    public function open()
    {
        ...
        switch ((int)$order->type) {
            case 1:
                $this->openEvent(Setting('new_order_event_id', 0));
                break;
            case 2:
                $this->openEvent(Setting('renew_order_event_id', 0));
                break;
            case 3:
                $this->openEvent(Setting('change_order_event_id', 0));
                break;
        }
    ...
    public function setOrderType(User $user)
    {
        ...
        if ($order->period === 'reset_price') {
            ...
            if (!(int)Setting('plan_change_enable', 1)) abort(500, '目前不允许更改订阅，请联系客服或提交工单操作');
            $order->type = 3;
            if ((int)Setting('surplus_enable', 1)) $this->getSurplusValue($user, $order);
    ...
    public function setInvite(User $user):void
    {
        ...
        switch ((int)$inviter->commission_type) {
            case 0:
                $commissionFirstTime = (int)Setting('commission_first_time_enable', 1);
        ...
        if ($inviter && $inviter->commission_rate) {
            ...
            $order->commission_balance = $order->total_amount * (Setting('invite_commission', 10) / 100);
        }
    }
